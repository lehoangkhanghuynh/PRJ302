    ...
    public ArrayList<UniversityDTO> searchByColum(String column, String value) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int toltalStudents = rs.getInt("totalStudents");
                int totalFaculties = rs.getInt("totalFaculties");
                boolean isDraft = rs.getBoolean("isDraft");

                UniversityDTO u = new UniversityDTO(id, name, shortName, description, foundedYear, address, city, region, type, toltalStudents, totalFaculties, isDraft);
                result.add(u);
            }
        } catch (Exception e) {
        }
